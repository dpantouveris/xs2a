...
public class SpiAspspConsentDataProviderImpl implements SpiAspspConsentDataProvider {
    ...
    private byte[] lastKnownData;

    @Override
    public byte[] loadAspspConsentData() {
        byte[] readedData = aspspDataService.readAspspConsentData(encryptedConsentId)
                                     .map(AspspConsentData::getAspspConsentData)
                                     .orElse(null);
        lastKnownData = readedData;
        return readedData;
    }

    @Override
    public void updateAspspConsentData(byte[] aspspConsentData) {
        if (Arrays.equals(lastKnownData, aspspConsentData)) {
