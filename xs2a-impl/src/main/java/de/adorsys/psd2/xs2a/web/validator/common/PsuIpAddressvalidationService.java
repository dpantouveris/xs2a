...
public class PsuIpAddressvalidationService {
     ...
     */
    public ValidationResult validatePsuIdAddress(HttpServletRequest request) {

        String psuIpAddress = request.getHeader(PSU_IP_ADDRESS);

        if (Objects.isNull(psuIpAddress)) {
            return ValidationResult.invalid(ErrorType.COMMON_400, TppMessageInformation.of(MessageErrorCode.FORMAT_ERROR, ERROR_TEXT_NULL_IP_ADDRESS));
        }

        if (isNonValid(psuIpAddress)) {
            return ValidationResult.invalid(ErrorType.COMMON_400, TppMessageInformation.of(MessageErrorCode.FORMAT_ERROR, ERROR_TEXT_WRONG_IP_ADDRESS));
        }
